var aayleyonline_nav = {
    doNotification: function() {
        ...
        browser.notifications.create('notifyON' + aayleyonline_params.title, { type: "basic", title: aayleyonline_params.title, message: aayleyonline_params.message, iconUrl: "iconon128.png" }, function(id) { });
    },
    setIconON: function(on) {
        var status = on ? "on" : "off";
        browser.browserAction.setIcon({path : "icon" + status + "48.png"});
    },
    goIt: function() {
        if(aayleyonline.isON){
            browser.tabs.create({url:aayleyonline.getCurrentRedirectUrl()},function(tab){});
        } else {
            browser.tabs.create({url:aayleyonline_params.offlineUrl},function(tab){});
        }
    }
}
